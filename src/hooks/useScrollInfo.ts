...
function useResizeObserver<T = any>(ref: MutableRefObject<T>, callback: Function) {
  useEffect(() => {
    // @ts-ignore
...
function useScrollInfo<T = any>(): [{
  x: Partial<ScrollPos>;
  y: Partial<ScrollPos>;
}, (node: any) => void, MutableRefObject<T>] {
  const [scroll, setScroll] = useState<{ x: Partial<ScrollPos>, y: Partial<ScrollPos>}>({ x: {}, y: {} });
